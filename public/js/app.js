    ...
    if (saveBtn) {
      saveBtn.onclick = async () => {
        ...
        try {
          ...
          if (updated) {
            this.currentShareStatus = updated;
            ...
            if (accessSelect) accessSelect.value = updated.is_shared ? 'public' : 'restricted';
            if (accessIcon) accessIcon.innerHTML = updated.is_shared ? ICON_GLOBE : ICON_LOCK;
            if (accessHint) {
              accessHint.textContent = updated.is_shared
                ? 'Anyone on the internet with this link can view and download'
                : 'Only people logged into TeleDrive can access this file';
            }
            if (publicSettings) {
              publicSettings.style.display = updated.is_shared ? 'block' : 'none';
            }
            if (revokeBtn) {
              revokeBtn.style.display = updated.is_shared ? 'inline-flex' : 'none';
            }
            if (linkInput) {
              linkInput.value = (updated.is_shared && updated.share_url) ? updated.share_url : '';
            }
            ...
            if (expStatus && expSelect) {
              if (updated.share_expires_at) {
                const expDate = new Date(updated.share_expires_at);
                const now = new Date();
                const isExpired = expDate < now;
                expStatus.innerHTML = isExpired
                  ? `<span style="color:#ea4335;">⚠️ Expired on ${UI.formatFullDateTime(updated.share_expires_at)}</span>`
                  : `<span style="color:#34a853;">● Active:</span> Expires on ${UI.formatFullDateTime(updated.share_expires_at)}`;
              } else {
                expSelect.value = 'never';
                expStatus.innerHTML = '<span style="color:var(--text-muted);">● Link never expires</span>';
              }
          ...
          UI.showToast(isShared ? 'Public sharing enabled! Share link is ready.' : 'File sharing turned off.', 'success');
          this.refreshCurrentView();
        } catch (err) {
    ...
    if (revokeBtn) {
      revokeBtn.onclick = async () => {
        ...
        try {
          ...
          this.currentShareStatus = { is_shared: 0, has_password: false, share_token: null, share_url: null };
          ...
          this.clearPasswordRequested = false;
          UI.showToast('Public link turned off successfully', 'info');
          this.refreshCurrentView();
        } catch (err) {
